      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredReports.map((report) => (
            <div
              key={report.id}
              className="bg-neutral-900/50 backdrop-blur-sm rounded-xl p-6 border border-neutral-800 hover:border-neutral-700 transition-all flex flex-col justify-between f"
            >
                ...
                {report.image && (
                  <div className="flex justify-center items-center">
                    <img
                      src={report.image}
                      alt="Report"
                      className="mt-4 rounded-lg object-contain max-h-40  border border-neutral-800"
                    />
                  </div>
                )}
              </div>
              <div className="mt-4">
                <select
                  ...
                  className="w-full bg-neutral-900 border border-neutral-800 text-neutral-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/20"
                >
                  {Object.values(ReportStatus).map((status) => (
                    <option key={status} value={status}>
                      {status}
